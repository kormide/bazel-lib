...
async function main(argv) {
  ...
  writeMetadataFile(bcrEntryPath);
  mkdirSync(bcrVersionEntryPath);
  writeModuleFile(projectPath, bcrVersionEntryPath);
  writeSourceFile(projectPath, bcrVersionEntryPath, ownerSlashRepo);
...
function writeMetadataFile(bcrEntryPath, projectPath) {
...
function writeModuleFile(projectPath, bcrVersionEntryPath) {
  const modulePath = resolve(projectPath, ".bcr", "MODULE.template.bazel");
  const moduleContent = readFileSync(modulePath, { encoding: "utf-8" });
...
async function writeSourceFile(
  ...
  const newSourceContent = sourceContent
    ...
    .replace("OWNER_SLASH_REPO_PLACEHOLDER", projectOrgSlashRepoName)

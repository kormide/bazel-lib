...
async function main(argv) {
  ...
  writeMetadataFile(bcrEntryPath, version);
  mkdirSync(bcrVersionEntryPath);
  writeModuleFile(projectPath, bcrVersionEntryPath, version);
  writeSourceFile(projectPath, bcrVersionEntryPath, ownerSlashRepo, version);
...
function writeModuleFile(projectPath, bcrVersionEntryPath, version) {
  const modulePath = resolve(projectPath, ".bcr", "MODULE.template.bazel");
  const moduleContent = readFileSync(modulePath, { encoding: "utf-8" });
